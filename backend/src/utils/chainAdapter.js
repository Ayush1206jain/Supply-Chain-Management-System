 ...
 * @returns {Promise<{contentHashOnChain: string, ownerAddress: string, registeredAt: number, status: number, lastUpdatedAt: number}|null>}
 */
async function getProductFromChain(product) {
  ...
  try {
    const productIdBytes32 = idToBytes32(product._id);
    // getProduct now returns: (contentHash, owner, registeredAt, status, lastUpdatedAt)
    const result = await client.contract.getProduct(productIdBytes32);
    const [contentHashBytes32, ownerAddress, registeredAtBigInt, statusUint8, lastUpdatedAtBigInt] = result;
    ...
    return {
      ...
      status: Number(statusUint8),          // 0=CREATED 1=IN_TRANSIT 2=DELIVERED 3=DISPUTED
      lastUpdatedAt: Number(lastUpdatedAtBigInt),
    };
  } catch (err) {
...
/**
 * Confirms a pending multi-sig ownership transfer on-chain.
 * Called by the receiver after `initiateTransfer` was already called by the sender.
 *
 * @param {object} product - Mongoose Product document (must have _id)
 * @returns {Promise<string|null>} tx hash or null
 */
async function confirmTransferOnChain(product) {
  const client = getChainClient();
  if (!client) {
    console.log("[chain] Blockchain not configured — skipping confirmTransfer");
    return null;
  }
  ...
  try {
    ...
    const tx = await client.contract.confirmTransfer(productIdBytes32);
    const receipt = await tx.wait();

    console.log(
      `[chain] Transfer confirmed on-chain for product ${product._id}. txHash: ${receipt.hash}`
    );
    return receipt.hash;
  } catch (err) {
    console.error(
      `[chain] confirmTransfer failed for product ${product._id}:`,
      err.message
    );
    return null;
  }
...
/**
 * Reads the pending transfer record from the contract (view — no gas).
 *
 * @param {object} product - Mongoose Product document (must have _id)
 * @returns {Promise<{from: string, to: string, initiatedAt: number, exists: boolean}|null>}
 */
async function getPendingTransferFromChain(product) {
  const client = getChainClient();
  if (!client) {
    console.log("[chain] Blockchain not configured — skipping getPendingTransfer");
    return null;
  }

  try {
    const productIdBytes32 = idToBytes32(product._id);
    const result = await client.contract.getPendingTransfer(productIdBytes32);

    return {
      from: result[0],
      to: result[1],
      initiatedAt: Number(result[2]),
      exists: result[3],
    };
  } catch (err) {
    console.error(
      `[chain] getPendingTransfer failed for product ${product._id}:`,
      err.message
    );
    return null;
  }
